  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => {
        ...
        setTotalReceivable(r.data.totalReceivable ?? 0)
      })
      .catch((err) => setError(err.response?.data?.message || 'Failed to load AR ledger'))
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-heading font-bold text-atc-text">AR Ledger</h1>
          <p className="text-sm text-atc-text-muted">Receivables (Ready to bill / Billed). Last 3 months.</p>
        </div>
      </div>
      ...
      {!error && (
        <>
          <div className="rounded-lg border border-atc-border bg-white p-4">
            <p className="text-sm text-atc-text-muted">Total receivable</p>
            <p className="text-2xl font-heading font-bold text-atc-text">
              {totalReceivable.toLocaleString('en-PH', { minimumFractionDigits: 2 })} PHP
            </p>
          </div>
                    ...
                    {ledger.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="py-8 text-center text-atc-text-muted">
                          No AR ledger entries for the period.
                        </td>
                      </tr>
                    ) : (
                      ledger.map((row) => (
                        <tr key={row.tripFinanceId} className="border-b border-atc-border hover:bg-atc-bg-subtle/50">
                          ...
                          <td className="py-3 px-4 text-atc-text-muted">{row.serviceCategoryName ?? '—'}</td>
                          <td className="py-3 px-4">{formatDate(row.runsheetDate)}</td>
                          <td className="py-3 px-4">{row.billingStatus?.replace(/_/g, ' ') ?? '—'}</td>
