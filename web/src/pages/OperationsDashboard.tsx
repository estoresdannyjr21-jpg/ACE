...
export function OperationsDashboard() {
  ...
  if (loading && !data) return <p className="loading-msg">Loading operations dashboard…</p>;
  ...
  return (
    <div>
      ...
      <section className="panel">
        ...
        <div className="filters-row">
          ...
          <div className="filter-group">
            ...
            <select className="filter-select" value={filters.clientAccountId ?? ''} onChange={(e) => setFilters((f) => ({ ...f, clientAccountId: e.target.value || undefined, serviceCategoryId: undefined }))}>
          ...
          <div className="filter-group">
            ...
            <select className="filter-select" value={filters.serviceCategoryId ?? ''} onChange={(e) => setFilters((f) => ({ ...f, serviceCategoryId: e.target.value || undefined }))} style={{ minWidth: 180 }}>
              <option value="">All</option>
          ...
          <div className="filter-group">
            ...
            <select className="filter-select" value={filters.operatorId ?? ''} onChange={(e) => setFilters((f) => ({ ...f, operatorId: e.target.value || undefined }))}>
              <option value="">All</option>
          ...
          <div className="filter-group">
            ...
            <select className="filter-select" value={filters.driverId ?? ''} onChange={(e) => setFilters((f) => ({ ...f, driverId: e.target.value || undefined }))} style={{ minWidth: 160 }}>
              <option value="">All</option>
          ...
          <div className="filter-group">
            ...
            <select className="filter-select" value={filters.assignmentStatus ?? ''} onChange={(e) => setFilters((f) => ({ ...f, assignmentStatus: e.target.value || undefined }))}>
              <option value="">All</option>
              {ASSIGNMENT_STATUSES.map((s) => (<option key={s} value={s}>{s.replace(/_/g, ' ')}</option>))}
            </select>
          </div>
          <div className="filter-group">
            <span className="filter-label">Trip status</span>
            <select className="filter-select" value={filters.highLevelTripStatus ?? ''} onChange={(e) => setFilters((f) => ({ ...f, highLevelTripStatus: e.target.value || undefined }))}>
              <option value="">All</option>
              {TRIP_STATUSES.map((s) => (<option key={s} value={s}>{s.replace(/_/g, ' ')}</option>))}
            </select>
          </div>
          <div className="filter-group">
            <span className="filter-label">POD status</span>
            <select className="filter-select" value={filters.podStatus ?? ''} onChange={(e) => setFilters((f) => ({ ...f, podStatus: e.target.value || undefined }))}>
              <option value="">All</option>
              {POD_STATUSES.map((s) => (<option key={s} value={s}>{s.replace(/_/g, ' ')}</option>))}
            </select>
          </div>
          <div className="filter-group">
            <span className="filter-label">Origin area</span>
            <input type="text" className="filter-input" placeholder="e.g. NCR" value={filters.originArea ?? ''} onChange={(e) => setFilters((f) => ({ ...f, originArea: e.target.value || undefined }))} style={{ minWidth: 100 }} />
          </div>
          <div className="filter-group">
            <span className="filter-label">Destination area</span>
            <input type="text" className="filter-input" placeholder="e.g. NCR" value={filters.destinationArea ?? ''} onChange={(e) => setFilters((f) => ({ ...f, destinationArea: e.target.value || undefined }))} style={{ minWidth: 100 }} />
          </div>
          <div className="filter-group">
            <span className="filter-label">Incident status</span>
            <select className="filter-select" value={filters.incidentStatus ?? ''} onChange={(e) => setFilters((f) => ({ ...f, incidentStatus: e.target.value || undefined }))}>
              <option value="">All</option>
              {INCIDENT_STATUSES_FILTER.map((s) => (<option key={s} value={s}>{s.replace(/_/g, ' ')}</option>))}
            </select>
          </div>
          <div className="filter-group">
            <span className="filter-label">Incident severity</span>
            <select className="filter-select" value={filters.incidentSeverity ?? ''} onChange={(e) => setFilters((f) => ({ ...f, incidentSeverity: e.target.value || undefined }))}>
              <option value="">All</option>
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 className="section-title">Pending acceptance</h2>
          ...
          <table className="table">
            ...
            <tbody>
              {data.pendingAcceptanceTrips.length === 0 ? (
                <tr><td colSpan={5} className="table-empty">No records</td></tr>
              ) : (
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 className="section-title">No update (call time +3h)</h2>
        <div className="table-wrap">
          <table className="table">
          ...
          <table className="table">
            ...
            <tbody>
              {data.openIncidents.length === 0 ? (
                <tr><td colSpan={6} className="table-empty">No records</td></tr>
              ) : (
                data.openIncidents.map((row) => (
                  <tr key={row.id}>
                    <td>{row.trip?.internalRef ?? '—'}</td><td>{row.incidentType}</td><td>{row.severity}</td><td>{row.status}</td>
